...
class FAISSStandardDataVectorStore:
    """FAISS-based vector store for standard building damage data"""

    def __init__(self, standard_data_path: str = "standard"):
        """
        Initialize the FAISS vector store with standard data

        Args:
            standard_data_path: Path to the standard data directory
        """
        self.standard_data_path = Path(standard_data_path)
        self.faiss_index_path = self.standard_data_path / "faiss_index"
        self.metadata_path = self.standard_data_path / "metadata.json"
    ...
    def _initialize_vectorstore(self):
        ...
        try:
            ...
            if self.faiss_index_path.exists() and self.metadata_path.exists():
                ...
                logger.info("Building new FAISS index from standard data...")
    ...
    def _build_vectorstore(self):
        """Build FAISS vector store from standard data files"""
        if not self.embeddings:
            logger.warning("Embeddings not available, skipping vector store build")
            return

        logger.info("Building FAISS vector store from standard data...")
...
def create_faiss_vector_store(
    ...
    try:
        return FAISSStandardDataVectorStore(standard_data_path)
    except Exception as e:
        ...
        vector_store.standard_data_path = Path(standard_data_path)
